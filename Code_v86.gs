/** 공감 원정대 v85 - 공용 결과 + 공용 한마디 게시판 API */
const RESULT_SHEET = 'RESULTS';
const COMMENT_SHEET = 'COMMENTS';
const ADMIN_PASSWORD = '39684475';
...
function doGet(e) {
  ...
  else data={ok:true,message:'공감 원정대 v85 API 정상',comments:true};
...
function doPost(e) {
  ...
  try{
    ...
    else if(action==='commentDelete') out=deleteComment_(p.commentId,p.password);
    else out={ok:false,message:'unknown action'};
  }catch(err){out={ok:false,message:String(err&&err.message||err)};}
  return ContentService.createTextOutput(JSON.stringify(out)).setMimeType(ContentService.MimeType.JSON);
}

function healthCheck(){ ensureSheets_(); return {ok:true,message:'공감 원정대 v85 API 정상',comments:true,time:new Date()}; }
...
function deleteComment_(id,password){
  if(String(password)!==ADMIN_PASSWORD)throw new Error('관리자 비밀번호가 올바르지 않습니다.');
  const sh=getCommentSheet_(),v=sh.getDataRange().getValues();
  for(let i=1;i<v.length;i++){
    if(String(v[i][0])===String(id)){sh.deleteRow(i+1);SpreadsheetApp.flush();return {ok:true,commentId:String(id)};}
  }
  throw new Error('삭제할 댓글을 찾지 못했습니다.');
}
