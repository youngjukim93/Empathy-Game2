...
function doGet(e) {
  ...
  else data={ok:true,message:'공감 원정대 API 정상'};
...
function healthCheck(){ ensureSheets_(); return {ok:true,message:'공감 원정대 API 정상',time:new Date()}; }
...
function saveComment_(c){
  ...
  const sh=getCommentSheet_(), id=String(c.id||Utilities.getUuid()); sh.appendRow([id,new Date(),String(c.mood||''),String(c.org||''),String(c.team||''),String(c.emp||''),String(c.name||''),Number(c.score||0),text]); return {ok:true,commentId:id};
}
